...
export default function Products() {
    ...
    const [products, setProducts] = useState(ALL_PRODUCTS); // Fallback to ALL_PRODUCTS initially
    ...
    useEffect(() => {
        const getSalesforceProducts = async () => {
            try {
                const response = await fetchProducts();
                if (response.data && response.data.products && response.data.products.length > 0) {
                    const sfProducts = response.data.products;
                    // Merge local and SF products to ensure all categories have items
                    ...
                    sfProducts.forEach(sfp => {
                        ...
                        else combined.push(sfp);
                    });
                    setProducts(combined);
                }
            } catch (err) {
                console.warn("Using local products as fallback:", err);
            }
        };
        getSalesforceProducts();
    }, []);
    ...
    const filtered = products.filter((p) => {
        ...
        if (!catMatch && p.cat) {
            ...
            catMatch = (pCat === aCat || pCat + 's' === aCat || pCat === aCat + 's');
        }
        const searchMatch =
    ...
    return (
        <div className="min-h-full pb-36" style={{ background: '#f0f4ff' }}>
